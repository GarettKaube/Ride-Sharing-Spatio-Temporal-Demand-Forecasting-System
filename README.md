...
Feature transforms such as logging, differencing, and cyclical transforms, were tested to try to improve performance. The models and performance were tracked using MLflow, with the final PyTorch Geometric Temporal model being wrapped in an MLflow PythonModel for easy model serving. With the model, the rest of the system design was layed out on AWS.

## Project Structure
rides.ipynb is the notebook with EDA, while modeling.ipynb is wher models were developed and tested. The src folder contains the code for AWS kinesis and Lambda. Scripts folder contains scripts to preprocess rides data and to partition the large rides data into multiple parquet files. Infrastructure contains Cloudformation templates and a script folder for creating the necessary Dynamodb tables. The graph_files folder contains files for the STGCN such as graph edge weights so that MLflow can use them. main.py is where the Kinesis stream is ran.
